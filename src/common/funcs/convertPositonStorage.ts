...
// Hàm lấy số cột và số hàng của mảng elements
export const calculateDimensions = (elements: number[], numCols: number) => {
	const [start, end] = [Math.min(...elements), Math.max(...elements)];
