...
import {PropsMainPageBillExport} from './interfaces';
import styles from './MainPageBillExport.module.scss';
...
function MainPageBillExport({}: PropsMainPageBillExport) {
	...
	const listBatch = useQuery(
		[QUERY_KEY.table_lenh_can_xuat, _page, _pageSize, _keyword, _customerUuid, _productTypeUuid, _status, _dateFrom, _dateTo],
		{
			queryFn: () =>
				httpRequest({
					isList: true,
				...
				<DataWrapper
					...
					noti={
						<Noti
							...
							onClick={() => router.push('/lenh-can/them-lenh-xuat')}
									...
									<div style={{display: 'flex', alignItems: 'center', gap: '4px'}}>
										...
										{data?.status < STATUS_BILL.DA_CAN_CHUA_KCS && data?.status != STATUS_BILL.DA_HUY ? (
											<IconCustom
												...
												tooltip='Chỉnh sửa'
												color='#777E90'
												href={`/lenh-can/chinh-sua-lenh-xuat?_id=${data.uuid}`}
											/>
										) : null}
...
export default MainPageBillExport;
