...
function MainPageBillTransfer({}: PropsMainPageBillTransfer) {
	...
	const listBatch = useQuery(
		[QUERY_KEY.table_lenh_can_chuyen_kho, _page, _pageSize, _keyword, _customerUuid, _productTypeUuid, _status, _dateFrom, _dateTo],
		{
			queryFn: () =>
				httpRequest({
					isList: true,
	...
	return (
		<div className={styles.container}>
			...
			<div className={styles.header}>
				...
				<div>
					<Button
						href={'/lenh-can/them-lenh-chuyen-kho'}
						p_8_16
				...
				<DataWrapper
					...
					noti={
						<Noti
							...
							onClick={() => router.push('/lenh-can/them-lenh-chuyen-kho')}
									...
									<div style={{display: 'flex', alignItems: 'center', gap: '4px'}}>
										...
										{data?.status < STATUS_BILL.DA_CAN_CHUA_KCS && data?.status != STATUS_BILL.DA_HUY ? (
											<IconCustom
												...
												tooltip='Chỉnh sửa'
												color='#777E90'
												href={`/lenh-can/chinh-sua-lenh-chuyen-kho?_id=${data.uuid}`}
											/>
										) : null}
...
export default MainPageBillTransfer;
