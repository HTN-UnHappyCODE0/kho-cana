...
function MainPageBillDirect({}: PropsMainPageBillDirect) {
	...
	const {_page, _pageSize, _keyword, _isBatch, _productTypeUuid, _shipUuid, _status, _dateFrom, _scalesStationUuid, _dateTo} =
		router.query;
	...
	const listBatch = useQuery(
		[
			QUERY_KEY.table_lenh_can_xuat_thang,
			_page,
			_pageSize,
			_keyword,
			_isBatch,
	...
	return (
		<div className={styles.container}>
			...
			<div className={styles.header}>
				...
				<div>
					<Button
						href={'/lenh-can/them-lenh-xuat-thang'}
						p_8_16
				...
				<DataWrapper
					...
					noti={
						<Noti
							...
							onClick={() => router.push('/lenh-can/them-lenh-xuat-thang')}
									...
									<div style={{display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '4px'}}>
										...
										{data?.status < STATUS_BILL.DA_CAN_CHUA_KCS && data?.status != STATUS_BILL.DA_HUY ? (
											<IconCustom
												...
												tooltip='Chỉnh sửa'
												color='#777E90'
												href={`/lenh-can/chinh-sua-lenh-xuat-thang?_id=${data.uuid}`}
											/>
										) : null}
...
export default MainPageBillDirect;
