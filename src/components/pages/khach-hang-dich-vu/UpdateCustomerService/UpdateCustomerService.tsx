...
import {IUpdateCustomerService, PropsUpdateCustomerService} from './interfaces';
import styles from './UpdateCustomerService.module.scss';
...
function UpdateCustomerService({}: PropsUpdateCustomerService) {
	const router = useRouter();

	const {_id} = router.query;

	const [form, setForm] = useState<IUpdateCustomerService>({
		name: '',
		userUuid: '',
		director: '',
		email: '',
	...
	const {data: detailCustomer, isSuccess} = useQuery([QUERY_KEY.chi_tiet_khach_hang, _id], {
		queryFn: () =>
			httpRequest({
	...
	const funcUpdateCustomer = useMutation({
		mutationFn: () =>
			httpRequest({
				...
				msgSuccess: `Chỉnh sửa khách hàng thành công!`,
				...
				<div className={styles.form}>
					...
					<div className={clsx('mt', 'col_3')}>
						<Select
							isSearch
							name='partnerUuid'
							placeholder={'Chọn khách hàng dịch vụ'}
