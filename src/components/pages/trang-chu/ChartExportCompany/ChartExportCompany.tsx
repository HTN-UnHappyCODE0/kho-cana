...
import {ITypeUu, PropsChartExportCompany} from './interfaces';
...
function ChartExportCompany({}: PropsChartExportCompany) {
	...
	const listStorage = useQuery([QUERY_KEY.dropdown_bai_xuat], {
		queryFn: () =>
			httpRequest({
				isDropdown: true,
	...
	useQuery(
		[
			QUERY_KEY.thong_ke_tong_hang_xuat,
			listCustomerUuid,
			storageUuid,
			date,
			listUserOwnerUuid,
